...
namespace sd_sockets {
  ...
  public:
    ...
    std::string read(const std::string &socketId, const std::chrono::steady_clock::duration & timeout = std::chrono::seconds(1)) {
    ...
    void write(const std::string &socketId, const std::string & msg, const std::chrono::steady_clock::duration & timeout = std::chrono::seconds(1)) {
    ...
    void accept(const std::function<void(Server *, const std::string &)> handler) {
      std::shared_ptr<asio::ip::tcp::socket> socket = std::make_shared<asio::ip::tcp::socket>(io_context_);
  ...
  private:
    std::vector<std::byte> read_exactly(std::string socketId, size_t n_bytes, const std::chrono::steady_clock::duration & timeout) {
